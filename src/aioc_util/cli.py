...
def setup_windows_dll() -> None:
    ...
    # Try loading from current directory first (user convenience)
    cwd_dll = os.path.join(os.getcwd(), "hidapi.dll")
    if os.path.exists(cwd_dll):
        try:
            getattr(ctypes, "WinDLL")(cwd_dll)
            return
        except OSError:
            pass

    # Try loading from script directory (for legacy/dev usage)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    dll_path = os.path.join(script_dir, "hidapi.dll")
    try:
        if os.path.exists(dll_path):
            getattr(ctypes, "WinDLL")(dll_path)
        else:
            # Hope it's in PATH
            getattr(ctypes, "WinDLL")("hidapi.dll")
    except OSError as e:
        logger.error("Could not load hidapi.dll: %s", e)
        sys.exit(1)


# Initialize Windows DLL setup
setup_windows_dll()
...
def read(device: Any, address: int) -> int:
...
def write_feat_report(device: Any, address: int, value: int) -> None:
    """Write a value to a register."""
    data = Struct("<BBBL").pack(0, Command.WRITESTROBE, address, value)
    device.send_feature_report(data)


def cmd(device: Any, cmd_val: int) -> None:
    """Send a command to the device."""
    data = Struct("<BBBL").pack(0, cmd_val, 0x00, 0x00000000)
    device.send_feature_report(data)


def dump(device: Any) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: aioc-util --ptt1 VPTT --store",
...
def parse_ptt_source(val: str) -> int:
    parts = val.split("|")
    return sum(PTTSource[p] for p in parts)


def parse_btn_source(val: str) -> int:
    parts = val.split("|")
    return sum(CM108ButtonSource[p] for p in parts)


def set_ptt_state_raw(device: Any, pin_num: int, state_on: bool) -> None:
...
def main() -> None:
    ...
    try:
        ...
        # hid.HIDException doesn't seem to be easily importable or varies, catching generic Exception or OSError
        logger.error(
            "Could not open AIOC device (VID: %#06x, PID: %#06x): %s",
            vid_open,
            pid_open,
            e,
        )
    ...
    try:
        ...
        if magic != b"AIOC":
            logger.error("Unexpected magic: %s", magic)
            sys.exit(-1)
        ...
        if args.swap_ptt:
            ...
            logger.info("Setting PTT1 Source to %s", p1)
            write_feat_report(aioc, Register.AIOC_IOMUX0, p1)
            logger.info("Setting PTT2 Source to %s", p2)
        ...
        if args.auto_ptt1:
            logger.info("Setting PTT1 Source to %s", PTTSource.VPTT)
        ...
        if args.ptt1 or args.ptt2:
            if args.ptt1:
                ...
                logger.info("Setting PTT1 Source to %s", PTTSource(val1))
                write_feat_report(aioc, Register.AIOC_IOMUX0, PTTSource(val1))
            if args.ptt2:
                val2 = parse_ptt_source(args.ptt2)
                logger.info("Setting PTT2 Source to %s", PTTSource(val2))
        ...
        if args.vol_up or args.vol_dn:
            if args.vol_up:
                ...
                logger.info("Setting VolUP button source to %s", CM108ButtonSource(su))
                write_feat_report(aioc, Register.CM108_IOMUX0, CM108ButtonSource(su))
            if args.vol_dn:
                sd = parse_btn_source(args.vol_dn)
                logger.info("Setting VolDN button source to %s", CM108ButtonSource(sd))
        ...
        if args.vptt_lvlctrl is not None:
            logger.info("Setting VPTT_LVLCTRL to %#x", args.vptt_lvlctrl)
        ...
        if args.vptt_timctrl is not None:
            logger.info("Setting VPTT_TIMCTRL to %#x", args.vptt_timctrl)
        ...
        if args.vcos_lvlctrl is not None:
            logger.info("Setting VCOS_LVLCTRL to %#x", args.vcos_lvlctrl)
        ...
        if args.vcos_timctrl is not None:
            logger.info("Setting VCOS_TIMCTRL to %#x", args.vcos_timctrl)
        ...
        if args.enable_hwcos:
            logger.info("Enabling hardware COS (if your aioc supports it)...")
            ...
            print(
                f"Now CM108_IOMUX0: {CM108ButtonSource(read(aioc, Register.CM108_IOMUX0))}"
            )
            print(
                f"Now CM108_IOMUX1: {CM108ButtonSource(read(aioc, Register.CM108_IOMUX1))}"
            )

        if args.enable_vcos:
            logger.info("Enabling virtual COS...")
        ...
        if args.foxhunt_get_message:
            ...
            for i, reg in enumerate(msg_registers):
                ...
                print(
                    f"  MSG{i}: {uint32_val:08x} ('{reg_bytes.decode('ascii', errors='replace')}')"
                )

            # Convert bytes to string, stopping at first null byte
            try:
                null_index = message_bytes.index(0)
                message_str = message_bytes[:null_index].decode(
        ...
        if (
            args.foxhunt_volume is not None
            or args.foxhunt_wpm is not None
            or args.foxhunt_interval is not None
        ):
            # Read current values
            current_foxhunt = read(aioc, Register.FOXHUNT_CTRL)
            current_volume = (current_foxhunt >> 16) & 0xFFFF
            ...
            new_volume = (
                args.foxhunt_volume if args.foxhunt_volume is not None else current_volume
            )
            new_wpm = args.foxhunt_wpm if args.foxhunt_wpm is not None else current_wpm
            new_interval = (
                args.foxhunt_interval
                if args.foxhunt_interval is not None
            ...
            logger.info(
                "Setting FOXHUNT_CTRL: volume=%d, wpm=%d, interval=%d",
                new_volume,
                new_wpm,
                new_interval,
            )
        ...
        if args.foxhunt_message is not None:
            ...
            message_bytes_in = args.foxhunt_message.encode("ascii", errors="replace")[
                :16
            ]
            message_bytes_padded = message_bytes_in.ljust(
                16, b"\x00"
            )  # Pad with nulls to 16 bytes
            ...
            logger.info("Setting foxhunt message: '%s'", args.foxhunt_message)
            for i in range(4):
                # Extract 4 bytes and convert to uint32 (little-endian)
                byte_offset = i * 4
                uint32_val = int.from_bytes(
                    message_bytes_padded[byte_offset : byte_offset + 4],
                    byteorder="little",
                )
                write_feat_report(aioc, msg_registers_w[i], uint32_val)
                print(
                    f"  MSG{i}: {uint32_val:08x} ('{message_bytes_padded[byte_offset:byte_offset+4].decode('ascii', errors='replace')}')"
                )
        ...
        if args.audio_get_settings:
            ...
            tx_boost_names = {TXBoost.TXBOOSTOFF: "off", TXBoost.TXBOOSTON: "on"}

            rx_gain_name = rx_gain_names.get(
                RXGain(current_rx), f"unknown ({current_rx:08x})"
            )
            tx_boost_name = tx_boost_names.get(
                TXBoost(current_tx), f"unknown ({current_tx:08x})"
            )
        ...
        if args.audio_rx_gain is not None:
            ...
            logger.info("Setting Audio RX gain to %s", rxgain.name)
        ...
        if args.audio_tx_boost is not None:
            boost_map = {"off": TXBoost.TXBOOSTOFF, "on": TXBoost.TXBOOSTON}
            txboost = boost_map[args.audio_tx_boost]
            logger.info("Setting Audio TX boost to %s", txboost.name)
